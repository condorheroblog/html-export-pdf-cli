...
export function createHtmlExportPdf(program: Command) {
	program
		...
		.option("--headless [headless]", "Whether to run the browser in headless mode", (v) => {
			if (v === "new")
				return v;
			if (v === "true")
				return true;
		...
		.option("--scale <scale>", "Scales the rendering of the web page. Amount must be between 0.1 and 2", v => Number(v), 1)
		...
		.option("--printBackground", "Set to print background graphics", false)
		.option("--omitBackground", "Hides default white background and allows generating PDFs with transparency", false)
		...
		.option("-l, --landscape", "Whether to print in landscape orientation", false)
		...
		.option("--ignoreHTTPSErrors", "Whether to ignore HTTPS errors during navigation", false)
